...
- Для влажности (70% соответствует "Высокой"): 
  - \(\mu_{\text{High}}(70) = 1\)
  - \(\mu_{\text{Medium}}(70) = 0\)
  - \(\mu_{\text{Low}}(70) = 0\)
...
#### Шаг 7: Дефазификация
Для получения четкого результата используем метод максимума или центроида. Например, если правило указало на мощность кондиционера:

\[
Output = \frac{\sum (output \times degree\_of\_truth)}{\sum degree\_of\_truth}
\]

В нашем случае:

\[
Output = \frac{50 \times 0.8}{0.8} = 50
\]
